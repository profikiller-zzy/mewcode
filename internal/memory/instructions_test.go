...
func TestLoadInstructionsBasic(t *testing.T) {
	...
	mustWrite(t, filepath.Join(dir, ".mewcode", "MEWCODE.md"), "dotdir instructions")

	out := LoadInstructions(dir)
	for _, want := range []string{"root mewcode rules", "root agents rules", "dotdir instructions"} {
...
// .mewcode/MEWCODE.md 和根目录的 MEWCODE.md 一样参与逐级遍历，
// 深层目录的那份排在浅层之后，优先级更高。
func TestLoadInstructionsDotDirWalksUp(t *testing.T) {
	...
	mustWrite(t, filepath.Join(root, ".mewcode", "MEWCODE.md"), "dotdir from root")
	mustWrite(t, filepath.Join(sub, ".mewcode", "MEWCODE.md"), "dotdir from leaf")

	out := LoadInstructions(sub)
	rootIdx := strings.Index(out, "dotdir from root")
	leafIdx := strings.Index(out, "dotdir from leaf")
	if rootIdx == -1 || leafIdx == -1 {
		t.Fatalf("both .mewcode files should appear; got:\n%s", out)
	}
	if rootIdx >= leafIdx {
		t.Errorf("leaf .mewcode file should be ordered after root; root=%d leaf=%d", rootIdx, leafIdx)
	}
}

// 同一目录下 MEWCODE.md 先于 .mewcode/MEWCODE.md 加载，
// 后者优先级更高。
func TestLoadInstructionsDotDirAfterPlain(t *testing.T) {
	dir := t.TempDir()
	mustInitGit(t, dir)
	mustWrite(t, filepath.Join(dir, "MEWCODE.md"), "plain file")
	mustWrite(t, filepath.Join(dir, ".mewcode", "MEWCODE.md"), "dotdir file")
	...
	if plainIdx >= dotIdx {
		t.Errorf(".mewcode file should be ordered after the plain one; plain=%d dot=%d", plainIdx, dotIdx)
	}
}
